...
(function (exports) {
   ...
   */
  function findName(name, stack,transformers) {
    var transforms = []
    if (name.indexOf('|')>0) {
    ...
    if (!!transformers) {
      for(var i=0;i<transforms.length;i++) {
        var t = trim(transforms[i]);
        if (!transformers[t]) return result;
        result = transformers[t](result);
      }
  ...
  function sendSection(send, value, callback, stack, inverted) {
    if (inverted) {
      ...
      if (value == null || value === false || value==='' || (isArray(value) && value.length === 0)) {
        send(callback());
      }
  ...
  function _compile(template, options) {
    ...
    return function (view, partials, callback) {
      ...
      var find = function (name) {
        var x = findName(name, stack,options.transformers);
        return x;
      };
